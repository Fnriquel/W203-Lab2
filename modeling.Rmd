...
```{r load model data}
model_df <- read.csv("model_df.csv") %>% filter(altitude_mean_meters_adj < 3000) %>% mutate(altitude_e100 = altitude_mean_meters_adj / 100) 
```
...
$$
```{r model1}
model1 <- lm(Total.Cup.Points ~ altitude_e100  + I(altitude_e100^2), data = model_df)
```
...
$$
Total.Cup.Points = \beta_0 + \beta_1 \cdot altitude\_e100 + \beta_2 \cdot altitude\_e100^2 + \beta_3 \cdot Non\_Tropical
$$

```{r model2}
model2 <- lm(Total.Cup.Points ~ altitude_e100 + I(altitude_e100^2) + Non_Tropical, data = model_df)
```
...
```{r model3}
model3 <- lm(Total.Cup.Points ~ altitude_e100 + I(altitude_e100^2) + Non_Tropical + total_defects
